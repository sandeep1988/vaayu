  ...
  def perform(trip_id, driver_id)
    ...
    if trip.assigned?
      ...
      if @notification.blank?
    	 Notification.create!(:trip => trip, :driver => driver,  :message => 'trip_should_start', :resolved_status => false, :new_notification => true, :reporter => 'ALYTE System').send_notifications
      end
    end
  end
end