  ...
  def send_trip_complete_sms
    ...
    if notification_channel[:sms]
      if self.status == 'completed'
        ...
        if @user.present?
          SMSWorker.perform_async(@user.phone, ENV['OPERATOR_NUMBER'], 'Your trip has been completed. Please use the ALYTE App to rate this trip, and optionally provide additional feedback.');
        end        
      end
    end    
  end
