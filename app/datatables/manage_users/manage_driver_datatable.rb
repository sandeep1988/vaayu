class ManageUsers::ManageDriverDatatable
  ...
  def data
    @site = Site.find_by_id(@driver.site_id) if @driver.site_id.present?
    {
        "DT_RowId" => @driver.id,
        :id => @driver.id,
        :name => @driver.user.present? && @driver.user.f_name.present? ? highlight_text([@driver.user.f_name.to_s, @driver.user.m_name.to_s, @driver.user.l_name.to_s].reject(&:blank?).join(" ")) : '' ,
        :f_name => @driver.user.present? && @driver.user.f_name.present? ? @driver.user.f_name.to_s : nil,
        # :m_name => @driver.user.m_name.to_s,
        :l_name => @driver.user.present? && @driver.user.l_name.present? ? @driver.user.l_name.to_s : nil,
        :email => @driver.user.present? && @driver.user.email.present? ? highlight_text(@driver.user.email) : nil,
        :phone => @driver.user.present? && @driver.user.phone.present? ? highlight_text(@driver.user.phone) : nil,
        :username => @driver.user.present? && @driver.user.username.present? ? @driver.user.username : nil ,
        # :status => (@driver.user.present? && @driver.user.pending? && @driver.user.status.present?) ? @driver.user.status.titleize : "-",
        :status => (@driver.user.present? && @driver.user.status.present? ) ? @driver.user.status.titleize : "-" ,
        :invite_count => @driver.user.present? && @driver.user.invite_count.present? ? @driver.user.invite_count : nil,
        :last_active_time => @driver.user.present? && @driver.user.pending? ? "-" : (@driver.user.present? &&  @driver.user.last_active_time.present? ? @driver.user.last_active_time.strftime("%m/%d/%Y %H:%M") : "--"),
